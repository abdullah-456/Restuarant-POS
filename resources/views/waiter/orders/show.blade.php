...
<style>
    ...
    .qty-btn-sm.dec:hover { background: #fee2e2; }
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">
                            @if($order->order_type === 'dining')
                                <i class="fas fa-chair mr-1 text-gray-400"></i>{{ optional($order->table)->name ?? 'No table' }}
                        ...
                        @if($order->order_type === 'delivery')
                            <div class="mt-2 text-xs bg-blue-50 text-blue-800 p-2 rounded-lg border border-blue-100">
                                ...
                                <div class="mt-0.5"><i class="fas fa-map-marker-alt mr-1"></i><strong>Address:</strong> {{ $order->delivery_address ?? 'N/A' }}</div>
                            </div>
                        @endif
                    </div>
                    ...
                    <table class="min-w-full text-sm" style="table-layout:fixed;">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2.5 text-right font-semibold text-gray-900">Rs. {{ number_format($item->subtotal, 2) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="mt-4">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <div class="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-emerald-700">
                            ...
                            <span class="bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-semibold">{{ $newItems->count() }}</span>
                        </div>
                        <div class="h-px bg-gray-200 flex-1"></div>
                    </div>
                    <div class="overflow-x-auto">
                                ...
                                <tr class="bg-emerald-50/30">
                                    <td class="px-3 py-2.5">
                                        <div class="flex items-center gap-1.5 flex-wrap">
                                            ...
                                            <span class="px-1.5 py-0.5 bg-emerald-600 text-white text-[9px] font-black rounded-full uppercase">NEW</span>
                                            @if(!empty($item->added_at))
                                                <span class="text-[10px] text-gray-400">({{ \Carbon\Carbon::parse($item->added_at)->format('H:i') }})</span>
                                            @endif
                                        </div>
                                        @if($item->notes)
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between text-gray-600">
                        ...
                        <span class="font-medium text-gray-800">Rs. {{ number_format($order->service_charge_amount, 2) }}</span>
                    </div>
                    @endif
        ...
        <div class="bg-white w-full sm:rounded-2xl shadow-2xl sm:max-w-5xl flex flex-col border border-gray-200"
            ...
            <div class="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
                ...
                <div class="md:w-3/5 flex flex-col border-b md:border-b-0 md:border-r border-gray-200 min-h-0"
                     style="max-height: 45vh; min-height: 0;"
                     id="menuPaneWrapper">
                    {{-- Search --}}
                    ...
                    <div class="flex-1 overflow-y-auto px-4 pb-4 space-y-4 custom-scrollbar min-h-0">
                        ...
                        <div class="modal-category-block">
                            <h5 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 sticky top-0 bg-white pt-1 pb-1 z-10">
                                {{ $category->name }}
                            </h5>
                            <div class="space-y-1.5">
                                ...
                                <button type="button"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-emerald-600 font-bold text-xs">Rs. {{ number_format($item->price, 0) }}</p>
                                    </div>
                                    ...
                                    <div class="w-7 h-7 rounded-full bg-gray-100 group-hover:bg-emerald-500 flex items-center justify-center transition flex-shrink-0"
                                        ...
                                        <i class="fas fa-plus text-gray-500 group-hover:text-white text-xs transition"></i>
                                    </div>
                                </button>
                                @endforeach
                            </div>
                ...
                <div class="md:w-2/5 flex flex-col bg-gray-50 min-h-0 overflow-hidden">
                    ...
                    <div class="px-4 py-3 bg-white border-b border-gray-200 flex-shrink-0 flex items-center justify-between">
                    ...
                    {{-- Summary stats --}}
                    <div id="selectedItemsSummary" class="px-4 py-2 bg-white border-b border-gray-100 hidden flex-shrink-0">
