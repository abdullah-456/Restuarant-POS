...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4">Edit Table</h2>
        ...
        <form method="POST" action="{{ route('admin.tables.update', $table) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="status" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @foreach(['available','occupied','reserved','cleaning'] as $status)
                        <option value="{{ $status }}" @selected(old('status', $table->status) === $status)>{{ ucfirst($status) }}</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-blue-600" @checked(old('is_active', $table->is_active))>
