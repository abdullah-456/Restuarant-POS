...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4">Edit Menu Item</h2>

        <form method="POST" action="{{ route('admin.menu-items.update', $item) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $item->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @if($item->image)
                        <p class="text-xs text-gray-500 mt-1">Current: <span class="underline">{{ $item->image }}</span></p>
                    @endif
                </div>
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-blue-600" @checked(old('is_active', $item->is_active))>
