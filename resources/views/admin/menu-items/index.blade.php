...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <a href="{{ route('admin.menu-items.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm md:text-base shadow-sm hover:bg-blue-700 transition">
                    ...
                    <tr>
                        <td class="px-3 md:px-4 py-2 md:py-3">
                            <div class="flex items-center gap-3">
                                @if($item->image)
                                    <img src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->name }}" class="w-10 h-10 rounded object-cover hidden sm:block">
                                @endif
                                <div>
                        ...
                        <td class="px-3 md:px-4 py-2 md:py-3">${{ number_format($item->price, 2) }}</td>
                        <td class="px-3 md:px-4 py-2 md:py-3">
                            @if($item->is_active)
                        ...
                        <td class="px-3 md:px-4 py-2 md:py-3 text-right">
                            <div class="inline-flex gap-2">
                                <a href="{{ route('admin.menu-items.edit', $item) }}" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded text-xs font-medium text-gray-700">
                                    Edit
                                </a>
                                <form id="del-item-{{ $item->id }}" action="{{ route('admin.menu-items.destroy', $item) }}" method="POST" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                <button type="button"
