...
<head>
    ...
    <style>
        ...
        .nav-link.active { background: rgba(255,255,255,0.18); color: white; box-shadow: inset 3px 0 0 #34d399; }
...
<body class="bg-gray-50">
    ...
    <div class="sidebar fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-blue-900 to-blue-800 shadow-2xl z-50 flex flex-col" id="sidebar">
        <div class="px-4 py-5 border-b border-white/10 flex-shrink-0">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 sidebar-text">
                    <div class="w-9 h-9 bg-gradient-to-br from-emerald-400 to-teal-600 rounded-xl flex items-center justify-center shadow-lg">
                        <i class="fas fa-person-walking text-white text-sm"></i>
                    </div>
                    <div>
                        <h1 class="text-base font-bold leading-tight">POS System</h1>
                        <p class="text-[10px] text-white/40 uppercase tracking-widest">Waiter</p>
                    </div>
                </div>
        ...
        <nav class="flex-1 overflow-y-auto py-4 space-y-0.5">
            <div class="px-4 mb-1">
                <p class="text-[10px] uppercase tracking-widest text-white/30 font-semibold px-2 sidebar-text">Menu</p>
            </div>
            <a href="{{ route('waiter.dashboard') }}" class="nav-link {{ request()->routeIs('waiter.dashboard') ? 'active' : '' }}">
                <i class="fas fa-home"></i><span class="sidebar-text">Dashboard</span>
            </a>
            <a href="{{ route('waiter.orders.index') }}" class="nav-link {{ request()->routeIs('waiter.orders.index') ? 'active' : '' }}">
                <i class="fas fa-list"></i><span class="sidebar-text">My Orders</span>
            </a>
            <a href="{{ route('waiter.orders.create') }}" class="nav-link {{ request()->routeIs('waiter.orders.create') ? 'active' : '' }}">
                <i class="fas fa-plus-circle"></i><span class="sidebar-text">New Order</span>
            </a>
        </nav>

        <div class="flex-shrink-0 p-4 border-t border-white/10 bg-black/20">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-8 h-8 bg-gradient-to-br from-emerald-400 to-teal-600 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    {{ Str::upper(Str::substr(auth()->user()->name, 0, 1)) }}
                </div>
                <div class="sidebar-text min-w-0">
                    <p class="text-sm font-semibold truncate">{{ auth()->user()->name }}</p>
                    <p class="text-xs text-white/40">Waiter</p>
                </div>
            </div>
            ...
            <div class="px-4 md:px-6 py-3 md:py-4">
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg md:text-xl font-bold text-gray-900">@yield('page-title', 'Dashboard')</h2>
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="text-xs text-gray-400 hidden sm:inline">{{ now()->format('D, d M Y') }}</span>
                        <div class="flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1.5">
                            ...
                            <span class="text-xs font-medium text-gray-700 hidden sm:inline">{{ auth()->user()->name }}</span>
                        </div>
                    </div>
                </div>
            </div>
