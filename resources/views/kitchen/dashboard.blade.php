    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="flex items-center gap-2 bg-orange-100 text-orange-700 px-4 py-2 rounded-xl font-semibold text-sm">
                    <i class="fas fa-fire-burner"></i>
                    <span>Live Orders</span>
                    <span id="count-current"
                        class="bg-orange-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">0</span>
                </div>
            </div>
...
@push('scripts')
    <script>
        ...
        let knownOrders = {}; // Store known orders and their "signature" (status + item count)
        ...
        function loadOrders() {
            ...
            fetch('{{ route("kitchen.orders.list") }}', { headers: { 'Accept': 'application/json' } })
                ...
                .then(orders => {
                    ...
                    // Check if anything actually changed to avoid flickering
                    const currentOrderIds = orders.map(o => o.id);
                    let hasChanges = false;

                    orders.forEach(order => {
                        const signature = `${order.status}-${(order.items || []).length}-${order.has_new_items}`;
                        if (knownOrders[order.id] !== signature) {
                            hasChanges = true;
                            knownOrders[order.id] = signature;

                            // If it has new items, auto print
                    ...
                    Object.keys(knownOrders).forEach(id => {
        ...
        function autoPrintOrder(orderId) {
            ...
            printFrame.src = `/admin/orders/${orderId}/print-kitchen?new_only=1&t=` + Date.now();
        }

        function markPrinted(orderId) {
            return fetch(`/kitchen/orders/${orderId}/mark-printed`, {
                method: 'POST',
                headers: {
